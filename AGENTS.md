...
This repository is the Codex-centered public-sector vibe-coding harness.
...
- The checker MCP command must be `gvskb-server` or `python -m gvskb.server`, never `gvskb mcp`. `.claude/.mcp.json` must be UTF-8 without BOM and include `PYTHONUTF8=1`, `PYTHONIOENCODING=utf-8`, and `GVSKB_MODE=offline`. If checker output includes non-null `profile_fallback`, mark validation incomplete.
