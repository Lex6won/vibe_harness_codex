...
3. `security-checker(full)`: `vibecode-checker(gvskb)` MCP로 full 점검을 실행하고, 체커가 저장한 HTML 리포트와 JSON 증적 2종을 최종 제출 기본자료로 확정한다. MCP를 쓸 수 없으면 미실행 사유와 대체 점검을 기록하고 제출 준비 완료로 처리하지 않는다.
4. `submit-packager`: 최종 리포트 2종을 보안팀 또는 AX 전담팀에 제출해야 함을 안내한다.
5. 기관 양식, 예외, 패키지 검토, 운영팀 설치 인계가 필요한 경우에만 `deploy-doc-writer`와 `submit-packager`가 `07_서버설치_배포가이드.md`, `08_배포신청서.md`, `09_예외신청서.md`, `10_패키지예외신청서.md`를 생성한다.

배포·이관 준비 모드는 개발 성공을 승인으로 표현하지 않는다. “최종 리포트 2종 제출 준비 완료/누락 있음/사람 승인 필요”로만 표현한다. 체커 리포트는 공식 승인서가 아니라 보안 검토 요청 증적이다.
